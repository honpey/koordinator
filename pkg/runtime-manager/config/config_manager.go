...
func (m *Manager) getAllRegisteredFiles() []string {
	...
	m.Lock()
	defer m.Unlock()
	for filepath, _ := range m.configs {
...
func (m *Manager) needRefreshConfig(filepath string) bool {
	...
	if lastUpdateTimestamp.Before(fileModTime) {
		return true
	}
	return false
...
func (m *Manager) Setup() error {
	if _, err := os.Stat(defaultRuntimeHookConfigPath); os.IsNotExist(err) {
		klog.Infof("%v not exist, to create", defaultRuntimeHookConfigPath)
...
func (m *Manager) collectAllConfigs() error {
	...
	for _, item := range items {
		...
		if err := m.updateHookConfig(filepath.Join(defaultRuntimeHookConfigPath, item.Name())); err != nil {
			klog.Errorf("fail to load config %v", err)
			continue
		}
	}
	return nil
...
func (m *Manager) syncLoop() error {
	klog.Infof("enter the fsnotify event handler")
	for {
		select {
...
func (m *Manager) healthCheck() {
	wait.Until(func() {
		...
		klog.Infof("current runtime hook config infos %v(%v)", allFiles, len(allFiles))
